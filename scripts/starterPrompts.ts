...
import { FIXTURE_DIR, availableCapabilities, loadFixtures } from "../src/eval/fixtures";
...
/**
 * Where the prompts came from, derived rather than written out — the fixture directory moved to
 * `eval/fixtures-wave1` for the rebuild and moves back at the end of it, and a hard-coded string
 * would silently misattribute the file across both moves.
 */
const SOURCE_DIR = path.relative(process.cwd(), FIXTURE_DIR).split(path.sep).join("/");
...
export const renderDocument = (prompts: StarterPrompt[], sensor: boolean): string => {
  const document: StarterPromptsDocument = {
    note: `Generated by \`npm run starter:prompts\` from ${SOURCE_DIR}. Do not edit by hand.`,
    source: SOURCE_DIR,
    sensorTool: sensor,
    prompts,
  };
