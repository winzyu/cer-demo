...
- `dev` was merged again on 2026-09-24 (EPA SOP re-resolve, catalogue 2026-09-24.1); labels regenerated byte-identical, both fingerprints unchanged (`EVAL_REBUILD.md`, frozen-set section).
- Calibration tooling and packet are built: `--run` on `grade:packet` and judge calibration, packet at `eval/grading/p3-calib-2026-09-24/warm/` (`EVAL_REBUILD.md` "Calibration packet").
...
1. Calibration (2c): the user grades `eval/grading/p3-calib-2026-09-24/warm/scores.csv` (correctness and ungrounded) without opening `KEY.json`; then, with spend approval, `npm run judge -- --run=p3-calib-2026-09-24 --calibration` (about $0.45) and `--calibrate` for kappa against 0.70.
...
3. About September 27 (`dev` is already merged; merge again only if it gains prompt or eval changes): final two-arm capture at k=20 with the Task A wording, judged twice with `--final`, then a tools-on live smoke check of the three previously failing questions plus one report question (live production reads, so get approval first).
