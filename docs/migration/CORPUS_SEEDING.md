...
| `documents/keyestudio-ks0414-turbidity-sensor.md`, `turner-turbidity-plus-sensor.md` | check `git ls-files documents/` at seeding time | only if the turbidity ingest chain (packet C8) has landed by then; confirm before assuming they are in the artifact |
