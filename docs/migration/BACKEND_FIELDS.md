...
Acted on 2026-08-20, in the report pipeline only -- see `DEVICE_API.md` §8b for what changed, the
band cut points, and the system-prompt change that stays deferred.
...
label. Before shipping continuity, decide deliberately: does inheriting a buoy inherit its data?
This needs an answer from the operator, not a default from us.
