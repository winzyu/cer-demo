...
Written 2026-09-17, packet C1 of `SUPPORTING_DOCS_TASKS.md` (archived at tag `old-machine-recovery-2026-09-19`).
...
The obvious framing is that v2's event signatures assert pollution causes, which is the class of claim `src/catalogue/catalogue.json` already gates behind supervisor approval (`docs/SPECS.md` §4b: only approved entries reach customers, and none is approved yet).
...
The prompt's citation rule in `systemPrompt.ts` lets the model quote any CONTEXT excerpt verbatim and present it as the answer; the only restrictions are on treating a document range as this pod's configured threshold and on quantitative turbidity claims.
Nothing there requires that a quoted cause or limitation statement first have passed the catalogue's approval, version, or `guidance_ids` audit trail (`GILLIGAN_TARGET_ARCHITECTURE.md` D4 and §2a, the `audit` field on each saved message).
...
Neither item is marked resolved anywhere in the tree searched for this memo (`GILLIGAN_PRODUCT_DIRECTION.md`, `STAKEHOLDER_QUESTIONS.md`).
...
1. The supervisor has closed stakeholder items 17-20 and approved catalogue entries covering the event, natural-cycle, and fault content v2 would otherwise duplicate (`docs/STAKEHOLDER_QUESTIONS.md`).
