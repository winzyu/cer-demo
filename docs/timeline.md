...
> What each stream built is in `SPECS.md` (§10.4, §15a); what remains is porting them to the
> dashboard page rebuilt in R3. The list here is kept as the phase's scope, not as a to-do.
...
| Pod access follows organization membership; history only where ownership is known | 2026-09-23 | User decision, closing the per-pod grant design (archived, tag `docs-tier2-archive-2026-09-23`). A user belongs to one organization and sees that organization's pods. A replaced pod's earlier readings are read only when the caller's own `/devices` shows the predecessor in the same organization; anything else is withheld and named. This is what `src/devices/mergeChains.ts` already did, so no code changed. A read-only live call the same day found every visible pod's organization present and `/devices` no longer returning retired predecessors, so all predecessor history is withheld today (`SPECS.md` §10.3c). Whether the three cross-organization merges transferred the site is `STAKEHOLDER_QUESTIONS.md` item 22. |
| Task C preserves answer-local tool evidence and audits citation edits | 2026-09-24 | Implemented and controlled-test verified across three `task/c-provenance` worktrees. Each invocation has a `Tn` handle, retained arguments/result/round/reuse metadata, and a link from the answer; relay storage and reopened history preserve it. Numeric quote markers are corrected only for a unique verbatim excerpt match; original answers, corrections and invalid markers remain auditable before display cleanup. Future capture and grading inputs carry tool evidence; existing transcripts are unchanged. Only tools-only prompt blocks changed, leaving the evaluated general prompt for R4. No production access, paid calls or deployment. Contract: `SPECS.md` section 10.4a; evidence and limits: `migration/TASK_C_VERIFICATION.md`. |
| September 30 holds with reduced scope; the user deploys | 2026-09-24 | User decision. The release keeps its date and cuts scope rather than slipping: catalogue content ships only if the supervisor approves in time (the user is chasing), otherwise `CATALOGUE_PROMPT` stays off. The user holds owner/editor access to the production Google Cloud project and executes R5 personally, so the runbook's separate upstream-owner and IAM-owner roles collapse to the user. The upstream organization's owners already know about the 2026-09-19 malware incident. |
| R1 remainder starts after Task C lands; its release scope is still open | 2026-09-24 | Task C's edits to `CerRagService.ts`, `GilliganController.ts`, `GilliganService.ts`, `ChatController.ts` and `ChatOrchestrator.ts` overlap R1's, so R1 is cut from the post-Task C heads. Open R1 parts: the `/gilligan/answer` contract (optional under D8), the service identity check (D12), a persistent usage store, and a model-call limiter with 429/503 retry. Image packaging is not an R1 row but blocks deployment: `.dockerignore` excludes `data/`, so the image carries neither the corpus nor the embedding cache. Packaging and the cer-demo side of the identity check touch no Task C file. Which of the rest must ship on September 30 is undecided. |
| Turbidity stays qualitative: 345/795 bands kept, all-zero periods flagged, NTU dropped (provisional) | 2026-09-24 | User decisions in the Task A interview, approved as provisional defaults pending sign-off. (1) The report's 345/795 edges stay: they are exactly the operator's 2.2 V and 0.7 V through `(3.35 - V) × 300`, so the dial's 350/800 is the discrepancy (stakeholder item 14), to fix upstream in `user-dashboard`. (2) A period in which every turbidity reading is 0 is flagged as a possible missing sensor in chat and reports; a lone 0 stays Clear, because `turbVoltToNTU.ts` returns 0 for a missing voltage and the offline sentinel as well as for clear water (item 10), and a live read on 2026-09-22 returned 14 days of zeros from a reporting pod. (3) The index has no unit and is never called NTU. (4) Every pod is treated as qualitative only until further notice, although the operator described the Turner sensor as quantitative on 2026-09-10; its datasheet claims no calibration (items 3, 18). Supervisor item 18 is treated as granted, so bands and caveat are unchanged. (5) Only cer-demo changed; returning null from `turbVoltToNTU.ts` and moving the dial to 345/795 are upstream follow-ups. `SPECS.md` §10.2 |
| Supervisor answers: release scope, limits, chat features and data policy | 2026-09-24 | The goal's five answer types and the no-cross-organization rule are confirmed. Usage limits are 20 messages and 5 reports per user per day plus a token cap still to be set, adjustable after testing. A question Gilligan cannot answer well is refused rather than answered weakly, which settles D3 toward refusal. Chat ships a pod picker, the question kept visible while waiting, formatted tables, source citations, a near-limit message and saved history, with further additions reviewed at the demo. Every answer carries "Content is AI generated, be sure to double check answers, turbidity is qualitative". Chats are kept indefinitely. Plan: `migration/GILLIGAN_RELEASE_PLAN.md` |
| Supervisor answers: backend access and deployment | 2026-09-24 | Live-backend writes are allowed for test users and organizations this project creates and deletes before launch. A new GCP service for cer-rag is allowed. New Firestore collections are allowed after the supervisor sees a table of the framework (stakeholder item 23). A release is deployed as a new version, tested from localhost, then given traffic. The `/water/period` membership hole is to be fixed (Task E). Testing uses the live backend. `migration/GILLIGAN_RELEASE_PLAN.md` |
| Supervisor answers: pod data | 2026-09-24 | The wide alert limits were unintentional and the supervisor corrects them in the superadmin view; until then a limit wider than the sensor's range reads "not assessed". The retired CWA Old pod still belongs to Cleveland Water Alliance, its old organization becomes null, and its readings stay on Old Woman Creek 2026. A pod that served several sites defaults to its current site in answers and reports unless the user asks otherwise. Every pod is Keyestudio; a sensor checkbox comes later. A failing turbidity sensor reads a flat 0 or 1005, which Gilligan should flag as abnormal. Stakeholder items 1-4, 10, 11, 18 and 22 |
| Catalogue approved; CER contact confirmed | 2026-09-24 | The supervisor approved every entry: the 26 marked up in the 2026-09-24 review as edited, the rest as written. Catalogue `2026-09-24.1` records the approvals, renames two referrals (Floating algae removal; Oil spill cleanup, small to medium), and gives every CER referral the contact "sales@cleanearthrovers.com, or your usual contact at Clean Earth Rovers", replacing the website's info address and booking link. Entries that route to CER support gained a `cer-support` referral carrying that contact. The approved professional-review selection needs "or duration over 12 hours", which the schema could not express, so `appliesTo.minDurationHours` was added. Two review notes outside the entries were not applied: renaming the customer-facing "Industrial" event label, and an optional inland conductivity entry. The release runs `CATALOGUE_PROMPT=true`. `SPECS.md` §4b |
| The user merges and deploys the upstream branches | 2026-09-24 | User decision. The dashboard and server feature branches are merged and deployed by the user, so no merge rights or third-party deploy steps are needed; the runbook is written for the user as the operator. |
| O6, O7 and S5 release decisions | 2026-09-24 | R1 ships S1-S6 as written; `/gilligan/answer` and identity tokens follow after launch. The payload ran on a build machine, and the user was told every credential has been rotated. Build and deploy only from clean feature branches with no build cache or image from before cleanup. CER replaces the cer-demo Fireworks key with its own paid key in Secret Manager. `migration/GILLIGAN_RELEASE_PLAN.md` |
| Token cap: 1,000,000 per user per day, as a guard | 2026-09-24 | User decision, answering O3 (was a 500,000 placeholder; 200,000 was also considered). The quota counts `usage.totalTokens` summed across tool rounds, cached prompt tokens included, so it measures volume, not spend. The R4 `p3-k20-2026-09-23` capture (hybrid-slice-vector, k=20, gpt-oss-120b, tools and catalogue off, 90 turns) averaged 19,211 prompt tokens, 8,620 of them cached, and 679 output tokens: about $0.0021 per turn at the `src/eval/prices.ts` rates. The release adds the catalogue (~5,300 tokens), the tool blocks (~2,200) and the tool definitions (~1,200) to the cached prefix, so a release message is estimated at 29,000 tokens with no tool call, 59,000 with one tool round and 90,000 with two, about $0.002-0.004. At those sizes 200,000 would allow 3-7 messages, below the 20-message cap; 1,000,000 lets the message cap bind on most days and still cuts off a runaway tool loop. The tools-on figures are estimates until E5 measures them. Fireworks serverless caches prompt prefixes automatically and per account, so all users already share the cached fixed prefix while `FIREWORKS_USER` stays constant; it offers no pre-warm or pin API. In the three R4 hybrid captures only the first request of a run missed that prefix (at most 1 of 90 turns). |
| Merged-pod history for a null-organization predecessor | 2026-09-24 | User decision, confirming the supervisor's answer on CWA Old. A retired pod whose registry organization is null is readable by the organization that owns the pod it was merged into, and by no one else; a predecessor registered to another organization stays withheld. Applies to the server's period-query membership check (release plan P3) and cer-demo's chain expansion (Q6). `CWA Old` today points at an organization id that does not exist (`migration/BACKEND_FIELDS.md` §5a); treating that like null is recommended and not yet decided. Marina Park and PCH Public Dock Buoy chains stay withheld until the supervisor answers. |
| A non-existent organization id counts as null; CWA Old merges into OWC 2026 | 2026-09-25 | User decision, relaying the supervisor's request, closing the question left open above. A retired pod whose registry organization id matches no organization document is treated exactly like one whose organization is null. Applies to the server follow-up to P3 and cer-demo's chain expansion (release plan Q6). |
| Pods report only on their current site | 2026-09-25 | Supervisor's request through the user, tightening the 2026-09-24 answer ("default to the current site, unless the user asks otherwise"). Chat answers, comparisons and reports use only readings from the pod's latest site; there is no option to ask for an earlier site, though an answer may say that earlier-location readings were excluded. With the CWA Old merge this matters most on Old Woman Creek 2026, whose merged chain is about 70% North Carolina and Utah rows (`migration/POD_RELOCATION_EVIDENCE.md` §6). Release plan Q3. |
| Test on the local Firestore mirror until live access; rotate keys after the demo | 2026-09-25 | User decision. The supervisor approved the Firestore table and the dedicated Gilligan database (F2); technical access is pending, so Firestore and behaviour fixes are verified against the local emulator mirror. CER's own Fireworks key replaces the cer-demo key, and credentials rotate, only after the user has tested and approved the demo (S5). The mirror's model calls have a $10 budget. |
