...
| R1 release scope, token cap and hosting | 2026-09-24 | User decisions. September 30 ships image packaging (S1), the service check (S2), the Firestore usage store (S3), the model-call retry and limiter (S4) and the near-limit flag (Q7); `/gilligan/answer` and identity tokens follow launch. The daily token cap starts at 1,000,000 per user, to be tuned in testing. cer-rag stays a separate Cloud Run service; merging it into the CER server is an after-launch item in `migration/GILLIGAN_RELEASE_PLAN.md` §5. |
| cer-rag service release implemented on `feat/service-release` | 2026-09-24 | The image ships `data/corpus/corpus.json` and `data/embeddings/cache.json` with `CORPUS_SOURCE=artifact`, checked against `release/artifacts.sha256` at build, from an allow-list `.dockerignore` (the old deny-list sent `.claude/`, `eval/` and an empty `serviceAccountKey.json` to the builder); the base image moves from Node 18 to 24 because `openai` 7 needs Node 22 or later. The service check is a shared `CER_RAG_SERVICE_KEY` rather than identity tokens, because Cloud Run strips the forwarded token's signature; the verified user now keys the quota. Quotas are daily in Firestore `gilligan_usage` with transactional updates, fail closed when the store is unreadable, and report `nearLimit` at 20% left. Fireworks 429/503 is retried once then answered `model_busy`; at most 8 model calls run at once. Server side: `clean-earth-rovers-server` `feat/service-key`, not pushed. Contract: `SPECS.md` §4a and §4c. Not yet run against Docker or the Firestore emulator. |
| Measured: token cost per question and saved chat size | 2026-09-24 | Five live questions through a local cer-rag with the release settings. Tokens: 18,137 for a document question, 38,194 for a report, 75,965 for one pod over 7 days, 129,241 for every pod's latest readings, 491,200 for a 30-day comparison across all pods, so the 1,000,000 cap binds before 20 questions for heavy users. Saved message: 5.6 KB to 179.7 KB, with `tool_calls` over 90% of the data-heavy ones, so about 5 heavy or 21 mixed messages fill Firestore's 1 MiB chat document. Recommendation and table: `migration/GILLIGAN_FIRESTORE_FRAMEWORK.md` "One risk to decide". |
