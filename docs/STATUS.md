...
- **Environment rebuild - done.** Corpus restored and byte-reproducible, secrets in, `/health` ok. `DEFAULT_RETRIEVAL=hybrid-slice-vector` with `CORPUS_SOURCE=artifact`, and `data/embeddings/cache.json` is built (7.1 MB, git-ignored). Per-item state: [`migration/LOCAL_STACK.md`](migration/LOCAL_STACK.md), "What git does not carry".
...
Independent tasks, written to be handed to separate sessions. None blocks another.
...
- **Task C - Chatbot next steps.** The assistant answers now, so the next gains are provenance and quality. The demo page (`frontend/js/*.js`) has all of the following; the dashboard page rebuilt in R3 has none of them yet, and the demo modules are the reference implementations. In value order:
  1. Widen `CerRagService.ts` to pass `tool_calls[].result` and `tool_round_cap_reached` through. Everything below is blocked on it, because the relay keeps only tool-call *names*.
  2. Provenance detail (`provenance.js`): one line per tool call, pod freshness, the tool's caveats, `complete: false`, with refusals styled as intentional.
  3. Series chart (`chart.js`): inline SVG, mean line with a min/max band, empty buckets omitted and never zero-filled.
  4. Input controls (`input.js`): multi-line composer, stop, copy, regenerate.
  5. The pod-status context bar (`podbar.js`), placed by the message-versus-chrome rule in `SPECS.md` §15a.
  6. Error codes (`SPECS.md` §7), then error UX built on them: "pod silent since Aug 7", never "no data" and never `0`.
  7. Time-range chips labelled Live / Week / Month / Year / 5 Years, the customer dashboard's own labels (`migration/DEVICE_API.md` §14c), which map one-to-one onto the device API's `:unit` ladder.
  8. A feedback loop (thumbs up/down becoming eval fixtures), blocked on where feedback is stored, which is the chat-history persistence question.

  **Citation markers on tool-derived claims.** The citation-placement defect below belongs to this task: the model cited a tool-derived claim with a non-supporting quote, and the prompt already forbids exactly that, so the question is why the rule was ignored. Two more live forms were seen on 2026-09-22 on `generate_report` answers through the relay: an empty `【】` ("shows **3 flagged events** (...)【】"), and `【?】` on each configured threshold. `MARKER_PATTERN` needs a digit, so both render as literal brackets. Do not simply hide them. Fix in this order:
  1. Give tool results a citable handle (for example `【T1】`), rendered as a provenance chip that opens that tool call. This needs item 1 above.
  2. Detect empty or out-of-range markers server-side, record them in the saved `audit`, and count them as `invalid_citations` in the eval gate. Treat every non-numeric marker alike.
  3. Only then, as a fallback, strip them on display, still recording the flag.

  The same answer typed `report_period`'s hyphens as U+2011 (defect row below), so any exact-match check on it should normalise hyphens first. What landed: [`migration/GILLIGAN_R3_PORT.md`](migration/GILLIGAN_R3_PORT.md), [`migration/GILLIGAN_TOOL_ACCESS.md`](migration/GILLIGAN_TOOL_ACCESS.md). Start with: `Read docs/STATUS.md, then work Task C, chatbot next steps.`
- **Task D - Doc consolidation, tier 2 - done 2026-09-23, uncommitted.** `CHAT_UX_WORKPLAN.md`, `migration/POD_AUTHORIZATION.md` and `migration/WSL_SANDBOX.md` are removed (tag `docs-tier2-archive-2026-09-23` on `9c783cd`, `ARCHIVED.md`); their live content moved to Task C above, `SPECS.md` §10.3c and §15a, `migration/SECURITY_FINDINGS.md` §1-§5, `STAKEHOLDER_QUESTIONS.md` item 22 and `migration/LOCAL_STACK.md`. `migration/GILLIGAN_PRODUCT_DIRECTION.md` stays, trimmed, because `catalogue.json` cites it. Pod access follows organization membership, and history is shown only where the organization is known to own it: the code already does this, and an approved read-only live call on 2026-09-23 confirmed every visible pod's organization and that retired predecessors are hidden, so all of them are withheld today. The commit and tag still need a Git plan.

## Last session

- Audited every tracked doc against the code and corrected stale statements in 27 files (routes, env, test counts, error codes, corpus totals, superseded plans, STATUS citations); committed with the pending Task B and eval-correction handoff edits as `9b331a8`.
- Archived tier 1 in `9c783cd`: `docs/advice/`, `EVAL_FIXTURE_QUALIFICATION.md`, `migration/INTEGRATION_PLAN.md`, `migration/codex-migration-report.md` (tags `docs-archive-2026-09-23`, `advice-archive-2026-09-17`).
- `documents/README.md` chunk column rebuilt from the corpus artifact (total 840,413 chars after the re-OCR); that defect row is closed.
- Checks: link and file-reference sweep only; docs-only, no code or tests changed. No spend, no live reads.
- Pushed `dev` (`703804d..9c783cd`) and both tags.

## Working tree

- `dev` is `9c783cd`, equal to `origin/dev`, plus this handoff's uncommitted `docs/STATUS.md` edit.
- `docs/EVAL_REBUILD.md` carries one uncommitted 23-line block (the 2026-09-21 claim re-resolve note), left out on purpose: that work lives on `worktree-eval-claims-reresolve` (`5d269a3`). `_EXIT_CRITERIA.md`, `eval/grading/` and the root v2 PDF stay untracked.
- Eval corrections are uncommitted in `.claude/worktrees/wave1-corrections` on `eval/wave1-corrections`.
...
8. Approve re-seeding Firestore (`npm run seed:firestore -- --prune` and the paid `seed:firestore-chunks -- --prune`): the collections predate the 2026-09-21 re-OCR, so the EPA SOP's text and 12 chunk ids are stale there, and the documents collection may still hold the removed document. Blocks: any `CORPUS_SOURCE=firestore` capture.
...
- The configured device token is **superadmin** (live read 2026-09-23) and sees 5 pods; the August census saw 15 because `/devices` no longer returns retired or merged pods. Its JWT carries **no `exp` claim**, so it never expires on its own.
...
- **Environment**: [`migration/LOCAL_STACK.md`](migration/LOCAL_STACK.md) (the checkout, what git does not carry, running dashboard and server locally), [`migration/SECURITY_INCIDENT_2026-09-19.md`](migration/SECURITY_INCIDENT_2026-09-19.md) (malware cleanup).
