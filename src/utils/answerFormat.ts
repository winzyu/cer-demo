 ...
 * in the system prompt** — not because the prompt is off-limits (the pin that used to justify that
 * was released 2026-08-26 when ◆G7 split, and the transcripts it protected were archived 2026-09-01
 * under `eval-archive-2026-09-01`; see `test/unit/prompt.test.ts`'s "the tool flags are additive"
 * block for what replaced it), but because the decision holds on its own merits: `EVAL_REBUILD.md`
 * Phase 2a made `【…】` markers a **deliberate, checked** part of the answer format
 * (verbatim-quote citations), so the prompt's job is to ask for good markers, not to apologise for
 * a bad one after the fact.
 */
